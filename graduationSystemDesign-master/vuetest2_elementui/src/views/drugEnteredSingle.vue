...
return Number(realVal)
  ...
  methods: {
    ...
    findMessage() {
      ...
      if (this.input1) {
        if (this.currentId == 1) {
          ...
          axios
            .get(
              "http://localhost:8181/drugEnteredDetails/findByNameContaining/" +
                _this.input1
            )
            .then(function (resp) {
              //console.log(resp);
          ...
          axios
            .get(
              "http://localhost:8181/drugEnteredDetails/findByProviderContaining/" +
                _this.input1
            )
            .then(function (resp) {
              //console.log(resp);
    ...
    resetRow() {
      const _this = this;
      axios
        .get("http://localhost:8181/drugEnteredDetails/findAll")
        .then(function (resp) {
          console.log(resp);
          alert("请重新查找");
          _this.input1 = null;
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:8181/drugEnteredDetails/findByEid/"+_this.$route.query.id)
      .then(function (resp) {
        console.log(resp);
        _this.tableData = resp.data;
        if(resp!=null){
