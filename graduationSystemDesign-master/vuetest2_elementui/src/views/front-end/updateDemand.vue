    ...
    export default {
        data() {
            return {
                ...
                rules: {
                     name: [
                        { required: true, message: '请输入需求药品信息', trigger: 'blur' },
                    ],
                     count: [
                        { required: true, message: '请输入需求药品数量', trigger: 'blur' },
                    ],
                     provider: [
            ...
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        axios.put('http://localhost:8181/demand/update',this.ruleForm).then(function (resp) {
                            if(resp.data=='success'){
                                // alert('添加成功')
                                // _this.$router.push('/pageOne')
                                _this.$alert( '需求药品:'+_this.ruleForm.name+' 数量信息已经修改并入库', '修改成功', {
        ...
        created() {
            ...
            const _this = this
            axios.get('http://localhost:8181/demand/findById/'+this.$route.query.id).then(function (resp) {
