...
export default {
  ...
  methods: {
    updateOrderState() {
      if (this.orderState == "等待处理") {
        console.log(this.tableData);
        axios.post("http://localhost:8181/book/updateCount/", this.tableData);
        axios.get(
          "http://localhost:8181/orderFront/updateOrderState/" +
            this.oid +
            "/" +
            "出库成功"
        );
        axios.post("http://localhost:8181/medicineSalesHandler/saveAll/",this.tableData);
        this.$alert("订单状态已经更改");
        this.$router.push({
          path: "/orderProcess",
        });
  ...
  created() {
    const _this = this;
    _this.oid = _this.$route.query.id;
    _this.orderState = _this.$route.query.state;
    axios
      .get(
        "http://localhost:8181/orderFrontDetails/findByOid/" +
