      ...
      <el-aside width="200px" style="background-color: rgb(238, 241, 246)">
        <el-menu router :default-openeds="open_list">
          ...
          >
            ...
            <el-menu-item
              @click="refresh()"
              v-for="(items2, index2) in item.children"
              :index="items2.path"
              :class="$route.path == items2.path ? 'is-active' : ''"
              >{{ items2.name }}</el-menu-item
            >
          </el-submenu>
        </el-menu>
      </el-aside>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        enddate: [
          { required: true, message: "请选择药品截止日期", trigger: "blur" },
        ],
        expirydates: [
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .put("http://localhost:8181/book/update", this.ruleForm)
            .then(function (resp) {
  ...
  created() {
    ...
    const _this = this;
    axios
      .get("http://localhost:8181/book/findById/" + _this.$route.query.id)
      .then(function (resp) {
