...
public interface MedicineReposity extends JpaRepository<Medicine_basicmessage,Integer> {
    ...
    @Query(nativeQuery = true,value="select * from medicine_basicmessage m WHERE m.name LIKE ? AND m.state=0 ")
    List<Medicine_basicmessage> findLikeName(String name);

    @Query(nativeQuery = true,value="select * from medicine_basicmessage m WHERE m.provider LIKE ? AND m.state=0 ")
    List<Medicine_basicmessage> findLikeProvider(String provider);
    ...
    @Query(nativeQuery = true,value="select * from medicine_basicmessage m WHERE TIMESTAMPDIFF(day ,m.enddate,NOW())>0 AND m.state=0")
